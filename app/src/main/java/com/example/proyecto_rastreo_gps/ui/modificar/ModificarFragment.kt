...
import com.example.proyecto_rastreo_gps.databinding.FragmentModificarBinding
...
class ModificarFragment : Fragment() {
    ...
    companion object {
        fun newInstance() = ModificarFragment()
    }
    ...
    override fun onCreateView(
        ...
        val textView: TextView = binding.textModificar

        modificarViewModel.text.observe(viewLifecycleOwner){
            textView.text = it
        }
        return root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.button.setOnClickListener {
            ...
            val first_name = binding.editTextText4.text.toString()
            val last_name = binding.editTextText5.text.toString()

            if (ruString.isBlank() || codCliString.isBlank() || name.isBlank() || first_name.isBlank() || last_name.isBlank()) {
            ...
            sendPutRequest(codCli.toString(), postRequest)
        }
    }

    private fun sendPutRequest(postId: String, postData: PersonaModelRequest) {
        viewLifecycleOwner.lifecycleScope.launch {
            clearTextViews()
            val response = service.updatePost(postId, postData)
            if (response.isSuccessful) {
                val postResponse = response.body()
                postResponse?.let {
                    binding.textView6.setText("${it.message}")
                } ?: run {
                    Log.e("Fragment_2_PUT", "Respuesta nula del servidor")
                }
            } else {
                val errorBody = response.errorBody()?.string()
                Log.e("Error de retrofit Fragment_2", "Error: ${response.code()} - ${response.message()} - $errorBody")
                binding.textView6.setText("Error: ${response.code()}")
            }
        }
    }
